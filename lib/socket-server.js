...
var socket_server = function () {
	...
	var socket_listen = function(port) {
		console.log('[socket-server] Starting WebSocket server.');
		...
		socket_domain.run(function() { 
			...
			socket_server.on('connection', function(socket) {
				...
				socket.on('message', function(data) { 
					console.log('[socket-server] Message received:', data);
				});
	...
	init = function(socket_port) {
		socket_listen(socket_port);
	};
