	...
	check_data : (data) => {
		...
		switch (data.msg[0]) {
			...
			case 0x02: // Broadcast: Device status
				data.command = 'bro';
				status[data.src.name].ready = true;
				...
				switch (data.msg[1]) {
					...
					case 0x01:
						status[data.src.name].reset = false;
						...
						if (data.src.name == 'DSP' || data.src.name == 'RAD') {
							setTimeout(() => {
								...
								if (config.emulate.bmbt === true) { omnibus.BMBT.power_on_if_ready(); }
								if (config.emulate.mid  === true) { omnibus.MID.power_on_if_ready();  }
							}, 1000);
						}
						break;
