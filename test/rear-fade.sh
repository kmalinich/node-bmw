...
while true; do
	...
	curl "${BASE}" -d "${TRL}&${SRL}" ; sleep ${SLEEP}
	curl "${BASE}" -d "${TRL}&${SRL}&${BRL}" ; sleep ${SLEEP}
	curl "${BASE}" -d "${TRL}&${SRL}&${BRL}" ; sleep ${SLEEP}
	curl "${BASE}" -d "${SRL}&${BRL}" ; sleep ${SLEEP}
	curl "${BASE}" -d "${SRL}&${BRL}" ; sleep ${SLEEP}
	curl "${BASE}" -d "${BRL}" ; sleep ${SLEEP}

	curl "${BASE}" -d "${TRR}" ; sleep ${SLEEP}
	curl "${BASE}" -d "${TRR}&${SRR}" ; sleep ${SLEEP}
	curl "${BASE}" -d "${TRR}&${SRR}&${BRR}" ; sleep ${SLEEP}
	curl "${BASE}" -d "${TRR}&${SRR}&${BRR}" ; sleep ${SLEEP}
	curl "${BASE}" -d "${SRR}&${BRR}" ; sleep ${SLEEP}
	curl "${BASE}" -d "${SRR}&${BRR}" ; sleep ${SLEEP}
