...
function process() {
	...
	if (msg_dst_name === 'UNKNOWN') {
		...
		if (debug === true) {
			console.log('[KBUS:PROT] error_current is now %s - invalid destination %s', error_current, msg_dst);
		}
		return 0;
	}
	...
	if (msg_src_name === 'UNKNOWN') {
		error_current = error_current+1;
		if (debug === true) {
			console.log('[KBUS:PROT] error_current is now %s - invalid source %s', error_current, msg_src);
		}
		return 0;
	}
	...
	if (calc_crc !== msg_crc) {
		error_current = error_current+1;
		if (debug === true) {
			console.log('[KBUS:PROT] error_current is now %s', error_current);
		}
		return 0;
	}
	...
	// Skip GLO destination messages from KBUS
...
module.exports = {
	...
	parser : (buffer) => {
		...
		if (input_buffer.length > 80) {
			if (debug === true) {
				console.log('[KBUS:PROT] Input buffer too large (%s); clearing', input_buffer.length);
			}
			error_current  = 0;
			input_buffer   = [];
			process_buffer = [];
		...
		if (error_current > 5) {
			if (debug === true) {
				console.log('[KBUS:PROT] Too many errors (%s); clearing input buffer', error_current);
			}
			error_current  = 0;
			input_buffer   = [];
			process_buffer = [];
		...
		if (debug === true) {
			console.log('[KBUS:PROT] PARSE OK - Splicing %s length', process_length);
		}
